    ...
    if (interaction.isCommand() || interaction.isContextMenu()) {
      ...
      if (!interaction.guild.members.me.permissions.has(Permissions.FLAGS.SEND_MESSAGES))
        return await interaction.user.dmChannel
          .send({
            content: `<:pristo_cross:1119465432588505129> | I don't have **\`SEND_interactionS\`** permission in <#${interaction.channelId}> to execute this **\`${SlashCommands.name}\`** command.`,
          })
          .catch(() => { });
      ...
      if (!interaction.guild.members.me.permissions.has(Permissions.FLAGS.EMBED_LINKS))
        return await interaction
          .reply({
            content: `<:pristo_cross:1119465432588505129> | I don't have **\`EMBED_LINKS\`** permission to execute this **\`${SlashCommands.name}\`** command.`,
            ephemeral: true,
          })
          .catch(() => { });
      ...
      if (!interaction.member.permissions.has(SlashCommands.botPrams || [])) {
        return await interaction.reply({
          content: `<:pristo_cross:1119465432588505129> | I Need Permission to Work this \`${SlashCommands.botPrams.join(', ')}\``,
          ephemeral: true,
        });
      }
      if (!interaction.guild.members.me.permissions.has(SlashCommands.userPrams || [])) {
        return await interaction.reply({
          content: `<:pristo_cross:1119465432588505129> | You Need this \`${SlashCommands.userPrams.join(
            ', ',
      ...
      if (SlashCommands.inVoiceChannel && !interaction.member.voice.channel) {
        return await interaction
          .reply({
            content: `<:pristo_cross:1119465432588505129> | You must be in a voice channel!`,
            ephemeral: true,
          })
          .catch(() => { });
      }
      if (SlashCommands.sameVoiceChannel) {
        if (interaction.guild.members.me.voice.channel) {
          if (interaction.guild.members.me.voice.channelId !== interaction.member.voice.channelId) {
            return await interaction
              .reply({
                content: `<:pristo_cross:1119465432588505129> | You must be in the same channel as ${interaction.client.user}`,
                ephemeral: true,
              })
              .catch(() => { });
          }
      ...
      if (SlashCommands.dj) {
        ...
        if (data) {
          if (data.Mode) {
            ...
            if (!pass && !interaction.member.permissions.has(perm)) return await interaction.reply({ content: `<:pristo_cross:1119465432588505129> | You don't have permission or dj role to use this command`, ephemeral: true })
          };
        };
      };
      try {
