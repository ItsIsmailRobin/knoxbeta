...
module.exports = {
  ...
  execute: async (message, args, client, prefix) => {
    ...
    if (pages.length <= 1) {
      ...
      let previousbut = new MessageButton().setCustomId('Previous').setLabel('<<').setStyle('SECONDARY');

      let nextbut = new MessageButton().setCustomId('Next').setLabel('>>').setStyle('SECONDARY');

      let stopbut = new MessageButton().setCustomId('stop').setEmoji('<:pristo_cross:1119465432588505129>').setStyle('DANGER');
